    ...
    <el-table :data="users" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
      ...
      <el-table-column label="操作" width="140">
        <template scope="scope">
          <el-button size="small" slot="reference" @click="showAccept(scope.$index, scope.row)">退款</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    refundOrder: function() {
      ...
      that.$http.post('/api/index/order/refund',para).then(function (res) { 
        that.visible1=false;
        if(res.data.code===200){
          that.$message({
            message: '退款成功',
